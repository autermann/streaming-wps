...
import com.github.autermann.wps.streaming.message.SOAPMessage.RelationshipType;
...
public abstract class AbstractMessageEncoding<T extends Message>
    ...
    private static final XmlOptions XML_OPTIONS = new XmlOptions()
            ...
            .setSaveSuggestedPrefixes(ImmutableMap.<String, String>builder()
                    ...
                    .put("http://www.opengis.net/wps/1.0.0", "wps")
                    .put("http://www.opengis.net/ows/1.1", "ows")
                    .put("http://www.w3.org/2005/08/addressing", "wsa")
                    .put("http://www.w3.org/1999/xlink", "xlink")
                    .put("https://github.com/autermann/streaming-wps", "stream")
                    .put("http://www.w3.org/2003/05/soap-envelope", "soap")
                    .build())
            .setSavePrettyPrint();
    ...
    public String encode(T message) throws XmlException {
        ...
        return document.xmlText(XML_OPTIONS);
    }
    ...
    private void encodeRelatedMessages(T message, Header header) {
        for (RelationshipType type : message.getRelatedMessages().keySet()) {
            for (Message.ID id : message.getRelatedMessages(type)) {
    ...
    protected void encodeCodeType(CodeType xbCodeType, OwsCodeType codeType) {
    ...
    private void decodeRelatesTo(T message, RelatesToDocument relatesToDocument) {
        ...
        if (uri.isPresent() && id.isPresent()) {
            ...
            if (type != null) {
                message.addRelatedMessages(type, Message.ID.create(id.get()));
            } else {
    ...
    private void decodeMessageId(T message, MessageIDDocument messageIDDocument) {
        Optional<Message.ID> id = decodeMessageID(decodeURI(messageIDDocument
                .getMessageID()));
        if (id.isPresent()) {
    ...
    protected Optional<Message.ID> decodeMessageID(Optional<URI> uri) {
        if (uri == null || !uri.isPresent()) {
            return Optional.absent();
        }
        return Optional.of(Message.ID.create(uri.get()));
    }

    protected Optional<Message.ID> decodeMessageID(AttributedURIType uri) {
        return decodeMessageID(decodeURI(uri));
    }

    protected StreamingProcess.ID decodeProcessID(ProcessIDType xbProcessId)
            throws XmlException {
        if (xbProcessId != null) {
            Optional<URI> uri = parseURI(xbProcessId.getStringValue());
            if (uri.isPresent()) {
                return StreamingProcess.ID.create(uri.get());
            }
        }
        throw new XmlException("Can not decode process id");
    }

    protected OwsCodeType decodeCodeType(CodeType codeType)
            throws XmlException {
        if (codeType == null) {
    ...
    protected void encodeData(DataType xbData, Data data)
    ...
    protected Data decodeData(DataType xbData) throws XmlException {
        if (xbData == null) {
